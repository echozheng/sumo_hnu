...
public class SumoTraciConnection {
	...
	public void runServer(boolean withGui) throws IOException, InterruptedException {
		...
		//在runSUMO方法中，已经对sumoProcess付初值：sumoProcess = Runtime.getRuntime().exec(argsArray);
		//InetAddress.getLocalHOst()默认是本地的IP
	...
	private void postConnect() throws IOException {
		...
		//通过发送命令（CMD_GET_EDGE_VARIABLE）,sumo返回相应的消息（消息还只是封装在dos中，并通过相应的类构造生成相应对象）
	...
	/**
	 * get the id list of the staffs which the @param command required
	 * 并把这些内容封装成 StringListQ
	 * **/
